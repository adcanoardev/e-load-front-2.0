...
const AboutHistory = () => {
    ...
    return (
        <Flex direction={{ base: "column", md: "row" }} width="100%" pt={10}>
            <Flex direction="column" alignItems="center" my={{ base: 8, md: 0 }} alignContent="center" p={10}>
                <Heading fontSize={30} marginBottom={10}>
                    Nuestra misión
                </Heading>

                {secondTexts.map((text, index) => (
                    <Text key={index} justify={"center"}>
                        {text}
                    </Text>
                ))}
            ...
            <Flex alignContent="center" direction="column" alignItems="center" p={10}>
                <Image
